		...
		try {
			...
			if(obj.getId() != 0) {
				PreparedStatement preparedStatement  = 
						Database.connexion.prepareStatement("UPDATE utilisateurs set nom=?, prenom=?, date_inscription=?, email=?, password=?, archiver=? WHERE id=?");
				preparedStatement.setString(1,obj.getNom());
				preparedStatement.setString(2,obj.getPrenom());
				preparedStatement.setString(3,obj.getEmail());
				preparedStatement.setDate(4,(Date) obj.getDate_inscription());
				...
				PreparedStatement preparedStatement  = Database.connexion.prepareStatement("INSERT INTO utilisateurs (nom, prenom, date_inscription, email, password, archiver) VALUES(?,?,?,?,?,?)");
				preparedStatement.setString(1,obj.getNom());
				preparedStatement.setString(2,obj.getPrenom());
				preparedStatement.setString(3,obj.getEmail());
				preparedStatement.setDate(4,(Date) obj.getDate_inscription());
...
public Utilisateur getById(int id) {
	try {
			...
			PreparedStatement preparedStatement  = Database.connexion.prepareStatement("SELECT * FROM utilisateurs WHERE id=? AND archiver=0");
...
public Utilisateur getOne(String name, String pass) {
	try {
		...
		PreparedStatement preparedStatement  = Database.connexion.prepareStatement("SELECT * FROM utilisateurs WHERE nom=? AND password=? AND archiver=0");
...
public Utilisateur getOneAndReviews(int id) {
	try {
		...
		PreparedStatement preparedStatement  = 
				Database.connexion.prepareStatement("SELECT * FROM `utilisateurs` LEFT JOIN commentaires ON utilisateurs.id = commentaires.fk_user WHERE utilisateurs.id=? AND commentaires.archiver=0;");
		preparedStatement.setInt(1, id);
	...
	public Utilisateur getOneAndCommandes(int id) {
		try {
			...
			PreparedStatement preparedStatement  = 
					Database.connexion.prepareStatement("SELECT * FROM `utilisateurs` LEFT JOIN commandes ON utilisateurs.id = commandes.fk_user WHERE utilisateurs.id=? AND commandes.archiver=0;");
			preparedStatement.setInt(1, id);
...
public Utilisateur getOneByNameAndMail(String name, String mail) {
	try {
		...
		PreparedStatement preparedStatement  = Database.connexion.prepareStatement("SELECT * FROM utilisateurs WHERE nom=? AND email=? AND archiver=0");
		preparedStatement.setString(1, name);
...
public ArrayList<Utilisateur> getAllNotArchived() {
	...
	try {
			...
			PreparedStatement preparedStatement  = Database.connexion.prepareStatement("SELECT * FROM utilisateurs WHERE archiver=0");
...
public ArrayList<Utilisateur> getAllArchived() {
	...
	try {
			...
			PreparedStatement preparedStatement  = Database.connexion.prepareStatement("SELECT * FROM utilisateurs WHERE archiver=1");
...
public void archiverById(int id) {
	try {
		
			PreparedStatement preparedStatement  = Database.connexion.prepareStatement("UPDATE utilisateurs set archiver=? WHERE id=?");
			preparedStatement.setInt(1,id);
