...
public class ContactDAO {
		...
		try {
			...
			if (obj.getId() != 0) {
				...
				PreparedStatement preparedStatement = Database.connexion.prepareStatement(
						"UPDATE contacts set fk_user=?, sujet=? , email=?, message=?, date=?, etat=?, archiver=? WHERE id=?");
				...
				PreparedStatement preparedStatement = Database.connexion.prepareStatement(
						"INSERT INTO contacts (fk_user, sujet, email, message, date, etat, archiver) VALUES(?,?,?,?,?,?,?)");
				preparedStatement.setInt(1, obj.getFk_user());
				preparedStatement.setString(2, obj.getSujet());
				preparedStatement.setString(3, obj.getEmail());
				preparedStatement.setString(4, obj.getMessage());
	...
	public Contact getByIdClient(int idCli) {
		try {
			...
			PreparedStatement preparedStatement = Database.connexion
					.prepareStatement("SELECT * FROM contacts WHERE fk_user=?");
	...
	public Contact getByIdClientAndEtat(int idCli, int statut) {
		try {
			...
			PreparedStatement preparedStatement = Database.connexion
					.prepareStatement("SELECT * FROM contacts WHERE fk_user=? AND etat=?");
			preparedStatement.setInt(1, idCli);
	...
	public ArrayList<Contact> getAllByEtat(int etat) {
		...
		try {
			...
			PreparedStatement preparedStatement = Database.connexion
					.prepareStatement("SELECT * FROM contacts WHERE etat=?");
	...
	public ArrayList<Contact> getAllByClient(int id) {
		...
		try {
			...
			PreparedStatement preparedStatement = Database.connexion
					.prepareStatement("SELECT * FROM contacts  WHERE fk_user=? ORDER BY contacts.date DESC");
			preparedStatement.setInt(1, id);
			ResultSet resultat = preparedStatement.executeQuery();
	...
	public void archiverById(Contact c) {
		try {
			...
			PreparedStatement preparedStatement = Database.connexion
					.prepareStatement("UPDATE contacts set archiver=? WHERE id=?");
	...
	public void deleteById(int fk_prod, int fk_user) {
		try {
			...
			PreparedStatement preparedStatement = Database.connexion
					.prepareStatement("DELETE FROM contacts WHERE fk_prod=? AND fk_user=?");
